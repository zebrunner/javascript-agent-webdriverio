...
The agent is not working automatically after adding into the project, it requires extra configuration. For this, you need to perform the following steps:
...
3. add ZebrunnerReporter to the list of reporters and provide reporter configuration (more about the configuration you can find in the next section). The reporter must be specified in a nested array. First array element is a reference on reporter class (not a string!), second element is a reporter configuration object. Here is the example of configuration snippet
...
4. add ZebrunnerService to the list of services. The service does not require additional configuration, but it must be specified as a reference on class (not as string!) and in a nested array of one element. Here is the example of configuration snippet
    ...
    // other configs
    ```
5. set the `reporterSyncInterval`. Sometimes, Zebrunner Agent requires extra time to submit all events to Zebrunner. To avoid lost events, it is recommended to set the value to at least `60 * 1000` (60 seconds).
...
| `REPORTING_LAUNCH_BUILD`<br/>`launch.build`                                  | This is the build number associated with the launch. It can reflect either the test build number or the build number of the application under test                                                             |
...
| `<N/A>`<br/>`launch.labels`                                                  | Object with labels to be attached to current launch. Property name is the label key, property value is the label value. Label value must be a string                                                           |
| `<N/A>`<br/>`launch.artifactReferences`                                      | Object with artifact references to be attached to current launch. Property name is the artifact reference name, property value is the artifact reference value. Value must be a string                         |
...
| `REPORTING_MILESTONE_NAME`<br/>`milestone.name` | Name of the Zebrunner Milestone to link the automation launch to. If the milestone does not exist, appropriate warning message will be displayed in logs, but the test suite will continue executing                               |
...
| Env var / Reporter config                                                               | Description                                                                                                                                                                                                                                                                                                                                                     |
|-----------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `REPORTING_NOTIFICATION_NOTIFY_ON_EACH_FAILURE`<br/>`notifications.notifyOnEachFailure` | Specifies whether Zebrunner should send notification to Slack/Teams on each test failure. The notifications will be sent even if the launch is still running. The default value is `false`                                                                                                                                                                      |
| `REPORTING_NOTIFICATION_SLACK_CHANNELS`<br/>`notifications.slackChannels`               | The comma-separated list of Slack channels to send notifications to. Notification will be sent only if Slack integration is properly configured in Zebrunner with valid credentials for the project the launch is reported to. Zebrunner can send two type of notifications: on each test failure (if appropriate property is enabled) and on the launch finish |
| `REPORTING_NOTIFICATION_MS_TEAMS_CHANNELS`<br/>`notifications.teamsChannels`            | The comma-separated list of Microsoft Teams channels to send notifications to. Notification will be sent only if Teams integration is configured in Zebrunner project with valid webhooks for the channels. Zebrunner can send two type of notifications: on each test failure (if appropriate property is enabled) and on the launch finish                    |
| `REPORTING_NOTIFICATION_EMAILS`<br/>`notifications.emails`                              | The comma-separated list of emails to send notifications to. This type of notification does not require further configuration on Zebrunner side. Unlike other notification mechanisms, Zebrunner can send emails only on the launch finish                                                                                                                      |

#### Logs

Zebrunner Reporter collects all logs produced using the `logLevel` framework (the framework comes bundled with WebdriverIO). The following configuration options control how logs will be submitted to Zebrunner.
...
| `REPORTING_LOGS_INCLUDE_LOGGER_NAME`<br/>`logs.includeLoggerName` | If value is set to `true`, then logger name will added in front of log message. For example, message `started` from logger `tests.e2e` will be submitted to Zebrunner as `tests.e2e: started`. The default value is `false` |
| `REPORTING_LOGS_EXCLUDE_LOGGERS`<br/>`logs.excludeLoggers`        | The comma-separated list of loggers that should be ignored by the agent logs collector                                                                                                                                      |
...
| Env var / Reporter config                                              | Description                                                                                                                                                                                                                               |
|------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `REPORTING_SCREENSHOT_BEFORE_COMMANDS`<br/>`screenshot.beforeCommands` | The comma-separated list of WebdriverIO command before which the Agent will take a screenshot of the browser or device. By default, the Agent does not take screenshots before any command                                                |
| `REPORTING_SCREENSHOT_AFTER_COMMANDS`<br/>`screenshot.afterCommands`   | The comma-separated list of WebdriverIO command after which the Agent will take a screenshot of the browser or device. The default list of commands is `'click', 'doubleClick', 'navigateTo', 'elementClick', 'scroll', 'scrollIntoView'` |
...
The WebdriverIO Agent is fully integrated with the Zebrunner Launcher and requires even less configuration when used with it. The Zebrunner Launcher automatically provides the `REPORTING_ENABLED`, `REPORTING_PROJECT_KEY`, `REPORTING_SERVER_HOSTNAME`, `REPORTING_SERVER_ACCESS_TOKEN` and some other environment variables, so there is no need to explicitly specify them or the corresponding `wdio.conf.js` file properties.
    ...
    capabilities: [
        {
            ...
            // another original capabilities from from the `wdio.conf.js` file...
        },
    ],
    // the service and reporter had to be configured before launch from Zebrunner Launcher
    services: [
        // ... other services
        [ZebrunnerService]
    ],
    reporters: [
        ...
        [
            ZebrunnerReporter,
            {
                ...
                // because Zebrunner Launcher provides all mandatory environment variables
            }
        ]
    ]
...
The Zebrunner Reporter collects all logs produced using the `logLevel` framework within a running test. If a message was logged within `#before()` or `#after()` section of a test suite, it will not be picked up by logs collector because these methods are invoked outside the scope of a test. If, according to `logLevel` settings or the Zebrunner Agent configuration, a message should not be logged, it will be ignored by the log collector.
...
Automatic screenshot capturing is possible after test failures and before or after a WebdriverIO command. To learn more how to configure automatic screenshot capturing refer to [Screenshots configuration section](#Screenshots).

Manually taken screenshot can be sent to Zebrunner using the `#saveScreenshot()` method of `currentTest` object. This method accepts any of the following arguments:
1. a `browser` instance
2. a `Promise` returned by `#takeScreenshot()` method of a `browser` instance
3. a base64-encoded `string` resolved from the `Promise` from previous option
4. a `Buffer` instance
...
You may want to add transparency to the process of automation maintenance by having an engineer responsible for evolution of specific tests or test suites. To serve that purpose Zebrunner comes with a concept of maintainer.

In order to keep track of those, the Agent comes with the `#setMaintainer()` method of `currentTest` object. This method accepts username of an existing Zebrunner user. If there is no user with the given username, `anonymous` will be assigned.
...
To attach label to test, you need to invoke `attachLabel` method of `currentTest` object in scope of the test method. To attach label to the entire launch, you can either invoke `attachLabel` method of `currentLaunch` object or provide the labels in [`wdio.conf.js` file](#Automation-launch-configuration).
...
describe('Test Suite', () => {
    ...
    before(function () {
        // will be attched to the entire run
...
Labels are not the only option for attaching meta information to test and launch. If the information you want to attach is a link (to a file or web page), it is more useful to attach it as artifact reference (or to put it simply, as a link).

The `attachArtifactReference` methods of `currentTest` and `currentLaunch` objects serve exactly this purpose. These methods accept two argument. The first one is the artifact reference name which will be shown in Zebrunner. The second one is the artifact reference value.
...
Zebrunner Agent comes with a convenient method `#revertRegistration()` of `currentTest` object for reverting test registration at runtime. The following code snippet shows a case where test is not reported on Monday.
